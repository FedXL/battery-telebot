...
class TextBuilder:
    ...
    def test_creator(self, username, first_name, second_name, patronymic, phone_contact, email, trading_point_name, trading_point,lottery_status):
        if self.language == 'rus':

            text = (f"Профиль продавца. <b>{username}</b>\n"
                        ...
                        f"Статус: <b>{lottery_status}</b>")

        elif self.language == 'kaz':

            text = (f"Сатушы профилі. <b>{username}</b>\n"
                        ...
                        f"Сауда нүктесінің атауы: <code>{trading_point_name}</code>\n"
                        f"Сауда нүктесінің мекенжайы: <code>{trading_point}</code>\n\n\n"
                        f"Жағдай: <b>{lottery_status}</b>")
        else:
...
async def profile_menu(callback: types.CallbackQuery , state: FSMContext, db: AsyncSession):
    ...
    text = TextConstrutor.test_creator(username=callback.from_user.username,
                                       ...
                                       lottery_status='Тут надо подумать что написать')

    await callback.answer('Profile')
    await callback.message.edit_text(text=text, reply_markup=keyboard)

router.callback_query.register(profile_menu, F.data == Calls.GO_TO_PROFILE)
